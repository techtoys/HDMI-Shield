...
/**
 * Known bug :  
 *  (1)	When Ra8876_Lite::bfc_DrawChar_RowRowUnpacked() is repeatly called, SD.open(pFilename) inside sometimes fail even if the file exists.
 *     	It is advised to copy all font data to SDRAM prior to main loop or move binary data to external serial Flash.
 *	(2) This program doesn't work with ESP32. Unwanted reset whenever any of the BFC related functions is called in loop(). 
 *		For example, when ra8876lite.getBfcStringWidth("/Brad60.bin" "Menu") is called in main loop() ESP32 would be reset.
 *		It has been traced down to SD.open() in BFC related functions that causes the unwanted reset.
 *		Searching over Internet and found one related thread:
 *		https://arduino.stackexchange.com/questions/31624/arduino-bug-when-myfile-sd-open-is-inside-loop
 *
 *		Some people commented that "You shouldn't repeatedly open the file, even if you close it in between. 
 *		Both the nature of flash based storage and the limitations of object oriented techniques in an 
 *		Arduino's tight memory mean you should avoid unecessary operations like that. 
 *		Also you should check the validity of the file object before you try to do operations on it"
 *
 *		J. Piquard commented that a state machine should be implemented in loop() to avoid unwanted reset.
 *		Trial pending for this...
 *
 */
